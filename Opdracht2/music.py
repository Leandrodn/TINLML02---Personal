...
◦ Building blocks: genomes consists of chords (3 notes) instead of singular notes, also the song ends in a II V I progression
◦ Mutatie: the offspring is mutated randomly by replacing a random chord in the genome with a random chord from the chord list
...
◦ Diversiteit: their are in total 7 different chords per pitch. 
...
def playMusic(melody, finished = False):
    ...
    thread.start()
    time.sleep(nrOfBars * nrOfChords) if not finished else time.sleep(nrOfBars * nrOfChords * 3)
    thread.terminate()
    time.sleep(1)
...
def makeSong(population):
    ...
    [song[0].append(note) for note in bestMelody[0]*3 + II_V_I[0]]
    [song[1].append(note) for note in bestMelody[1]*3 + II_V_I[1]]
    [song[2].append(note) for note in bestMelody[2]*3 + II_V_I[2]]
